...
我们先来看看需求2。因为番茄钟只会作为待办的预估出现，这也意味着只有在创建了待办事项（Doro）后，才有可能产生番茄（Pomo），所以我们可以将番茄作为待办事项的一个属性来进行管理。我设想理想的交互是，在界面上有专门的若干灰色番茄图案，（这也意味着我们限制了一个待办事项可能预估的上限番茄数量——鼓励用户将待办拆分成具体可量化的事项），用户可以在这些图案上点击或者滑动来调整预估的番茄钟数量。而区分三种预估的思路如下：
